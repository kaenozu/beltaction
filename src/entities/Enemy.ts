...
export class Enemy extends Entity {
  ...
  public downAttackImage: HTMLImageElement | null = null;
  ...
  private resolveHit(player: Player): void {
    ...
    if (player.canReceiveGroundHit || player.canBeKnockedDownByFollowup || (DebugFlags.allowPostGameOverAttacks && player.canReceivePostGameHit)) {
      ...
      hitResolved = true;
    } else if (this.state === 'bodyBlow' && (player.isBound || player.isGrabbed)) {
  ...
  private tryJoinGrabFollowup(player: Player): boolean {
    const canJoin = (player.isGrabbed || (player.isBound && player.boundReadyForFollowup)) && !this.isHoldingPlayer && !player.isDoubleGrabbed && this.attackCooldown <= 0;
